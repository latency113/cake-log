...
interface DepartmentSalesBarChartProps {
  departmentPounds: { id: string; name: string; totalPounds: number }[];
}

const DepartmentSalesBarChart: React.FC<DepartmentSalesBarChartProps> = ({
  departmentPounds,
}) => {
  ...
  const internalDepartmentColorMap: { [key: string]: string } = {};
  departmentPounds.forEach((dept) => {
    internalDepartmentColorMap[dept.name] = getDepartmentColor(dept.name);
  });
  
  const chartData = {
    labels: departmentPounds.map((data) => data.name),
    datasets: [
      {
        label: "จำนวนปอนด์รวม",
        data: departmentPounds.map((data) => data.totalPounds),
        backgroundColor: departmentPounds.map(data => internalDepartmentColorMap[data.name]),
        borderColor: departmentPounds.map(data => internalDepartmentColorMap[data.name]),
        borderWidth: 1,
        borderRadius: 4,
      },
    ],
  ...
  const chartOptions = {
    ...
    plugins: {
      ...
      datalabels: {
        ...
        // Only show background if not on mobile to save space
      ...
      y: {
        ...
        ticks: {
          ...
          callback: function (tickValue: string | number) {
            ...
            return (
              new Intl.NumberFormat("th-TH").format(tickValue as number) +
              " ปอนด์"
            );
          },
        },
        ...
        {departmentPounds.length > 0 ? (
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="text-center">
              <p className="text-slate-400 font-medium">ไม่มีข้อมูลจำนวนปอนด์</p>
            </div>
          </div>
        )}
      </div>
...
export default DepartmentSalesBarChart;